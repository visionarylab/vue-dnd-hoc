...
export default {
  ...
  move (state, payload) {
    ...
    target.left =
      left > 0 ? (left < 1280 - target.width ? left : 1280 - target.width) : 0;
    target.top =
      top > 0
        ? top < state.page.height - target.height
          ? top
          : state.page.height - target.height
        : 0;
  },
  ...
  replaceState (state, payload) {
    state.widgets = JSON.parse(JSON.stringify(payload['widgets']));
  },
